...
class Configuration extends MX_Controller {
    ...
    public function setStuFee() {
        ...
        $data['classFee'] = $this->common->getAllData('set_fees');
        $this->load->view('temp/header');
        $this->load->view('allClassFees', $data);
        $this->load->view('temp/footer');
    }

    //This function can configure new class accounts fees
    public function configClassFee() {
        if ($this->input->post('submit', TRUE)) {
            $insertData = array(
                'class_id' => $this->db->escape_like_str($this->input->post('Class_title', TRUE)),
                'admission' => $this->db->escape_like_str($this->input->post('admission', TRUE)),
                're_admission' => $this->db->escape_like_str($this->input->post('re_admission', TRUE)),
                'tuition' => $this->db->escape_like_str($this->input->post('tuition', TRUE)),
                'contributions' => $this->db->escape_like_str($this->input->post('contributions', TRUE)),
                'game' => $this->db->escape_like_str($this->input->post('game', TRUE)),
                'library_member_fee' => $this->db->escape_like_str($this->input->post('library_member_fee', TRUE)),
                'laboratory_charges' => $this->db->escape_like_str($this->input->post('laboratory_charges', TRUE)),
                'receipt' => $this->db->escape_like_str($this->input->post('receipt', TRUE)),
                'square_girls_guide' => $this->db->escape_like_str($this->input->post('square_girls_guide', TRUE)),
                'electricity' => $this->db->escape_like_str($this->input->post('electricity', TRUE)),
                'poor_fund' => $this->db->escape_like_str($this->input->post('poor_fund', TRUE)),
                'religion' => $this->db->escape_like_str($this->input->post('religion', TRUE)),
                'development_charge' => $this->db->escape_like_str($this->input->post('development_charge', TRUE)),
                'examination_fee' => $this->db->escape_like_str($this->input->post('examination_fee', TRUE)),
                'teacher_welfare_fund' => $this->db->escape_like_str($this->input->post('teacher_welfare_fund', TRUE))
            );
            if ($this->db->insert('set_fees', $insertData)) {
                redirect('configuration/setStuFee', 'refresh');
            }
        } else {
            $data['classTile'] = $this->common->getAllData('class');
            $this->load->view('temp/header');
            $this->load->view('newFeeForm', $data);
            $this->load->view('temp/footer');
        }
    }

    //This function can edit students fees by class wary
    public function editClassFee() {
        if ($this->input->post('submit', TRUE)) {
            ...
            $this->db->where('id', $id);
            if ($this->db->update('set_fees', $insertData)) {
                redirect('configuration/setStuFee', 'refresh');
            }
        } else {
            $id = $this->input->get('rid');
            $data['classTile'] = $this->common->getAllData('class');
            $data['fees'] = $this->configarationmodel->getClassFee($id);
            $this->load->view('temp/header');
            $this->load->view('editClassFee', $data);
            $this->load->view('temp/footer');
        }
    }

    //This function can delete clss fees
    public function classFeeDelete() {
        $id = $this->input->get('rid');
        $userId = $this->input->get('userId');
        if ($this->db->delete('set_fees', array('id' => $id))) {
            redirect('configuration/setStuFee');
        }
    }
    ...
    function test() {
        $query = $this->db->query("SELECT user_id,full_name FROM userinfo WHERE user_group_title = 'Accountant'");
        foreach ($query->result_array() as $row) {
            $data[] = $row;
        }
        echo '<option value="">' . lang('select') . '</option>';
