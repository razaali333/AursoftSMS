...
//        }
    ...
    public function chFiExRe($classTitle){
        $query = $this->db->query("SELECT publish,status FROM add_exam WHERE class_title='$classTitle' AND final='Final'");
        foreach ($query->result_array() as $row){
            $publich = $row['publish'];
            $status = $row['status'];
        }
...
//    
//    //This function will return students next class.
//    public function nextClass($classId){
//        $data = array();
//        $query = $this->db->query("SELECT id,class_title FROM class WHERE id < '$classId' ORDER BY id DESC LIMIT 1");
//        foreach ($query->result_array() as $row){
