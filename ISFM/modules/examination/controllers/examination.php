...
class Examination extends MX_Controller {
    ...
    public function addExam() {
        ...
        if ($this->input->post('submit', TRUE)) {
            ...
            $examTerm = array(
                'exam_term' => $this->db->escape_like_str($this->common->examTerm($class_id)),
                'exam_month' => $this->db->escape_like_str($this->input->post('examMonth', TRUE))
            );
            $this->db->where('class_id', $class_id);
            $this->db->update('set_fees', $examTerm);
    ...
    public function ajaxClassResult() {
        ...
        if (!empty($data)) {
            echo '<div class="form-group">
                        <label class="col-md-3 control-label">' . lang('exac_4') . ' <span class="requiredStar"> * </span></label>
                        <div class="col-md-6">
                            <select onchange="examSubject(this.value)" name="examID" class="form-control">
    ...
    public function ajaxExamSubject() {
        ...
        if (!empty($subject)) {
            echo '<div class="form-group">
                        <label class="col-md-3 control-label">' . lang('exac_8') . ' <span class="requiredStar"> * </span></label>
                        <div class="col-md-6">
                            <select onchange="examSubjectTitle(this.value)" name="subjRutID" class="form-control">
                            <option value="">' . lang('select') . '</option>';
            foreach ($subject as $sub) {
...
//                    $optional =  $this->exammodel->optinalSubject($classTitle, $examTitle);
//                    $finalPoint = $pointAverage - $optional;
...
//        echo '</pre>';
    ...
    function test() {
        $pre_point = 5;
        if ($this->exammodel->che_opt_sub('201501001', 'Amar Bangla Boi')) {
            $point = $pre_point - 2;
        } else {
            $point = $pre_point;
        }
