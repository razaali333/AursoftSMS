...
class ExamModel extends CI_Model {
    ...
    public function examTitle($id) {
        $data = array();
//        $query = $this->db->query("SELECT exam_id FROM exam_routine WHERE id=$id")->row();
...
//      $query = $this->db->get_where('result_shit', array('student_id' => $studentId));
    ...
    //This function will return bullian about a subject is that optional or not
//    public function optinalSubject($claTit,$subTit){
//        $data = array();
//        $query = $this->db->query("SELECT id FROM class_subject WHERE class_title='$claTit' AND subject_title='$subTit' AND optional=1");
//        foreach($query->result_array() as $row){
//            $data = $row['id'];
//        }
//        if (!empty($data)){
